...
\begin{equation}\label{constrainedparameters1}
  \theta_{1,1} < 0,\dots,\theta_{t,1} < 0,\dots,\theta_{T,1} < 0
  \quad \text{and} \quad
  \theta_{1,p} < 0,\dots,\theta_{t,p} < 0,\dots,\theta_{T,p} < 0.
\end{equation}
...
\end{equation}
$\displaystyle\theta_0(t) = -\log\left\{ \int_\mathds{R} \exp \left( \sum\limits_{k=1}^{p} \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y) \right)dy \right\}$ and $\theta_k(t) = \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t)$, $k=1,\dots,p$ where $\bm{\alpha} = \{\alpha_{j,k}\} \in \mathcal{A}$ are parameters for $j=0,\dots,q$ and $k=1,\dots,p$, and $\theta_0(t)$ is the normalising constant so that $\int_\mathds{R} f(y|t;\bm{\alpha})  dy = 1$.
...
\begin{equation}\label{constrainedparametersconditional}
  \alpha_{0,1} < 0,\dots,\alpha_{q,1} < 0
  \quad \text{and} \quad
  \alpha_{0,p} < 0,\dots,\alpha_{q,p} < 0.
\end{equation}
...
\begin{equation}\label{newconstrainedparametersconditional}
  \phi_{0,1} < \frac{1}{D^{(1)}_1(z)},\dots,\phi_{q,1} < \frac{1}{D^{(1)}_1(z)}
  \quad \text{and} \quad
  \phi_{0,p} < 0,\dots,\phi_{q,p} < 0
\end{equation}
...
\end{equation}
where $r_{-t}(\bm{\theta},h) = \log \left\{ \int_\mathds{R} \exp \left( {\sum\limits_{s(\ne t)=1}^{T} \sum\limits_{k=1}^{p} w_s(t) \theta_{s,k} B_k(y)} \right)  dy \right\}$. Using leave-one-out log-likelihood functions over all $T$ data sets, $\{ L_t(h) \}_{t=1}^T$, the optimal bandwidth $\hat{h}$ could be found by maximising the sum of the leave-one-out log-likelihood functions as in \eqref{method1}.
