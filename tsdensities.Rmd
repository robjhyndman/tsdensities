...
All existing density estimation methods handle continuous data; however, few methods handle discrete data. @LiRacine2003 considered the nonparametric estimation of joint distributions with mixed discrete and continuous data. @HallRacineLi2004 considered the nonparametric estimation of conditional densities with mixed data and used cross-validation for choosing optimal bandwidths. This paper will address this issue---limited research on handling discrete data in density estimation methods---by proposing four conditional density estimation methods with time as a discrete conditioning variable.
...
\end{equation}
$\theta_0(t) = -\text{log} \left\{ \int_\mathds{R} \text{exp} \left( \sum\limits_{k=1}^{p} \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y) \right)\text{d}y \right\}$ and $\theta_k(t) = \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t)$, $k=1,\dots,p$ where $\bm{\alpha} = \{\alpha_{j,k}\} \in \mathcal{A}$ are parameters for $j=0,\dots,q$ and $k=1,\dots,p$, and $\theta_0(t)$ is the normalising constant so that $\int_\mathds{R} f(y|t;\bm{\alpha}) \hspace{0.1cm} \text{d}y = 1$.
