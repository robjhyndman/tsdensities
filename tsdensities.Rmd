...
$$
  b_t(y;\bm{\theta}_t) = \sum\limits_{k=1}^{p_t} \theta_{t,k} B_{t,k}(y),\qquad
...
# Logspline density estimates with common knots
\label{ES1.1:sec:logsplinewithcommonknots}

Let $K (\geq 3)$ be the number of knots and let $\{z_1,\dots,z_K\}$ be a knot sequence with $-\infty < z_1 < \dots < z_K < \infty$. The knot sequence $\{z_k\}$ is the same for each time series. Let $S$ denote the collection of twice continuously differentiable functions $s$ on $\mathds{R}$ such that the restriction of $s$ to each of the intervals $(-\infty,z_1],[z_1,z_2],\dots,[z_{K-1},z_K],[z_K,\infty)$ is a cubic polynomial. Thus the functions in $S$ are cubic splines with knots at $z_1,\dots,z_K$. Let $S_0$ denotes the $K$-dimensional subspace of $S$ comprising functions $s \in S$ such that $s$ is linear on $(-\infty,z_1]$ and $[z_K,\infty)$. Set $p = K - 1$. Then $S_0$ has a basis set of the form $\{B_0(y)\equiv1,B_1(y),\dots,B_p(y)\}$, which can be chosen such that $B_1(y)$ is linear with a negative slope on $(-\infty,z_1]$; $B_2(y),\dots,B_p(y)$ are constant on $(-\infty,z_1]$; $B_p(y)$ is linear with a positive slope on $[z_K,\infty)$; and $B_1(y),\dots,B_{p-1}(y)$ are constant on $[z_K,\infty)$.

Let $\tilde{f}_t(y;\bm{\theta}_t)$ be the probability density estimate from data $\{y_{t,j}\}_{j=1}^{n_t}$ given $\bm{\theta}_t = \{\theta_{t,k}\}_{k=1}^p \in \Theta$. The logspline model at time $t$ is
\begin{equation}\label{mymodel}
  \tilde{f}_t(y;\bm{\theta}_t) = \exp \left( b_t(y;\bm{\theta}_t) - c_t(\bm{\theta}_t) \right)
\end{equation}
where $b_t(y;\bm{\theta}_t) = \sum\limits_{k=1}^{p} \theta_{t,k} B_k(y)$ and $c_t(\bm{\theta}_t) = \log \left\{ \int_\mathds{R} \exp \left( b_t(y;\bm{\theta}_t) \right) dy \right\}$.

The differences between \eqref{usualmodel} and \eqref{mymodel} are the number of knots ($K_t$ compared to $K$) and the basis functions ($B_{t,k}(y)$ compared to $B_k(y)$). Since the usual logspline density estimation approach treats every data set separately in \eqref{usualmodel}, the number of knots is different in each time series $t = 1,2,\dots,T$, and the basis functions are different. However, the logspline model in \eqref{mymodel} has a fixed number and fixed locations of knots in each time series $t$; hence, the basis functions are unchanging over time.

The optimal parameters at time $t$ are estimated by maximising the log-likelihood function of the logspline density function at time $t$:
\begin{equation}\label{maximumlikelihood}
  L_t(\bm{\theta}_t) = \sum\limits_{j=1}^{n_t} \log f_t(y_{t,j};\bm{\theta}_t).
\end{equation}
@Stone1990 showed that the log-likelihood of a univariate logspline density function is a strictly concave function on $\Theta$, and that the maximum likelihood estimate $\hat{\bm{\theta}}_t$ of $\bm{\theta}_t$ is unique if it exists.

## Common knots selection {#ES1.1:subsec:commonknotsselection}

By using the ideas of the knot placement for a single data set for logspline density estimation, we propose a new algorithm to select knots for multiple data sets in an optimal manner. This algorithm consists of four steps. First, the initial number of knots is set to $2(\bar{n})^{1/5}$ where $\bar{n}$ is the average number of observations of each data set $\{n_t\}_{t=1}^T$. This choice is theoretically supported using large sample inference for a logspline density estimator with common knots as $\big\lVert \hat{f}_t(y) - f_t(y) \big\rVert_2 = O \left( \bar{n}^{-\xi} \right)$ where $f_t(y) = f_t(y;\bm{\theta}_t)$ is the density function at time $t$, $\hat{f}_t(y) = f_t(y;\bm{\hat{\theta}}_t)$ is the logspline density estimate of $f_t(y)$, $\xi = \frac{d}{(2d+1)}$, and $d (> 0.5)$ depends on the smoothness condition of $f_t(y)$ [@Stone1990]. Hence, the number of initial knots is taken as $2\bar{n}^{1/5}$. The power $1/5$ is used based on a recommendation by @KooperbergStone1991, and the multiple $2$ is somewhat arbitrary and mainly as a result of experience.

Initial knots are placed in the second step of the algorithm. Originally, we placed two boundary knots at the first- and last-order statistics of the pooled data ($\text{min}(y)$ and $\max(y)$). However, because of the linear tail restrictions on the basis functions, we found that placing the boundary knots beyond the first- and last-order statistics works better in practice. We place $z_1 = \text{min}(y) - \lambda$ and $z_K = \max(y) + \lambda$ where $\lambda = (\max(y) - \text{min}(y))/4$. If a log transformation is considered for the density estimation and $z_1 = \text{min}(y) - \lambda$ is non-positive, we consider $z_1 =\text{min}(y)$. In this paper, we use a close approximation of the Chebyshev procedure [@DeBoor1978] to place the initial knots $K_{\text{int}}$, which splits the semicircle into $N-1$ arcs of equal length to select $N$ points over an interval $[\text{min}(y),\max(y)]$.

After obtaining the number and locations of the initial knots, the next step of the algorithm involves the stepwise addition of knots. The BIC criterion is used for the knot addition: $-2\log L(\cdot) + \tau p$ where $L(\cdot)$ is the log-likelihood function in \eqref{maximumlikelihood}, $p$ is the number of parameters in the model, and the penalty parameter $\tau = \log n$ where $n$ is the sample size. We calculate the BIC value for each data set and for each knot interval $(z_1,z_2),\dots,(z_{K-1},z_K)$. For each knot interval, the average BIC value over all data sets is then calculated. The potential knot with the smallest average BIC is added to the model. Knots are added until the maximum number of knots $3\bar{n}^{1/5}$ is reached where the multiple $3$ is chosen based on experience. In the final step, the least important knots are removed successively during stepwise knot deletion where the BIC is also used to measure importance. This procedure is repeated until there are five knots left.
...
In the method described in Section&nbsp;\ref{ES1.1:sec:logsplinewithcommonknots}, we estimate the densities independently for every year, ignoring all other years. Under normal circumstances, it is reasonable to assume that these univariate densities are related to the ones in nearby years, and it is natural to account the nearby years to the density estimation of a particular year. @MasseTruong1999 proposed a spline-based conditional logspline density estimation method which applies to bivariate data where both response and conditioning variables are continuous. In this section, we propose a modified version of the conditional method with two additional features: we adapt the method to allow for discrete support of the conditioning variable, time $T'$; and we optimize both the knots and the degree of the spline. The concept of selecting the optimal spline degree for $B$-spline basis functions has also been used in the R package `crs` [@RacineNieRipley2014].

Let $f(y|t)$ be the conditional density function of $Y$ given $T' = t$, where $f(y|t)$ is modelled using sets of basis functions on $Y$ and $T'$. We generate a set of $B$-spline basis functions on $Y$ as in Section&nbsp;\ref{ES1.1:sec:logsplinewithcommonknots} and specify the candidate polynomial degrees of the basis functions from 2 to 10. In contrast, the basis functions in the $T'$ dimension are treated as piecewise cubic polynomials with a fixed degree value 3, and they are generated as follows. Let $J (\geq 3)$ be the number of knots in the $T'$ dimension and let $-\infty < z'_1 < \dots < z'_J < \infty$ be a knot sequence. Set $q = J - 1$. Let $B$-spline basis functions in the $T'$ dimension be $G_0(t) \equiv 1,G_1(t),\dots,G_q(t)$. The boundary basis functions on $T'$ are the same as the ones on $Y$ so that $G_1(t)$ is linear with a negative slope on $(-\infty,z'_1]$; $G_2(t),\dots,G_q(t)$ are constant on $(-\infty,z'_1]$; $G_q(t)$ is linear with a positive slope on $[z'_J,\infty)$; and $G_1(t),\dots,G_{q-1}(t)$ are constant on $[z'_J,\infty)$.
...
$$
  \theta_0(t) = -\log\left\{
    \int_\mathds{R} \exp \left(
      \sum_{k=1}^{p} \sum_{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y)
    \right)dy
  \right\},
$$
and $\theta_k(t) = \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t)$, $k=1,\dots,p$ where $\bm{\alpha} = \{\alpha_{j,k}\} \in \mathcal{A}$ are parameters for $j=0,\dots,q$ and $k=1,\dots,p$, and $\theta_0(t)$ is the normalising constant so that $\int_\mathds{R} f(y|t;\bm{\alpha})  dy = 1$.

The log-likelihood function of the conditional logspline model with a discrete conditioning variable is defined by:
$$
  L(\bm{\alpha})
    = \sum_{t=1}^{T} \sum{i=1}^{n_t} \log f(y_{t,i}|t;\bm{\alpha})
    = \sum_{t=1}^{T} \sum{i=1}^{n_t} \left(
        \sum{k=1}^{p} \sum{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y_{t,i}) - a(\bm{\alpha})
      \right)
$$
where $\displaystyle a(\bm{\alpha}) = \log \left\{ \int_\mathds{R} \exp \left( \sum\limits_{k=1}^{p} \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y) \right)  dy \right\}$.

## Knots and spline degree selection {#ES1.2:subsec:knotsselection}

We choose the fixed number of knots in the $T'$ dimension, $J=5$.\todo{This seems arbitrarily specific. Why 5?} In the $Y$ dimension, we follow Section&nbsp;\ref{ES1.1:subsec:commonknotsselection} to find the initial number and locations of the knots. Having all initial knots plus the first added knot, the optimal spline degree is found by minimising BIC. After each knot addition, a search is made for the best spline degree from the candidate spline degrees from 2 to 10. Knots are added until the maximum number of knots $3\bar{n}^{1/5}$.
...
\begin{equation}\label{constrainedparametersconditional}
  \alpha_{j,1} < 0 \quad \text{and} \quad \alpha_{j,p} < 0,
\end{equation}
for $j=0,\dots,q$.
...
\begin{equation}\label{newconstrainedparametersconditional}
  \phi_{j,1} < \frac{1}{D^{(1)}_1(z)}
    \quad \text{and} \quad
  \phi_{j,p} < 0,
\end{equation}
where $j=0,\dots,q$, and $\bm{\phi} = \{\phi_{j,k}\} \in \Phi$ is a set of parameters on log space ($j=0,\dots,q; k=1,\dots,p$).

@MasseTruong1999 modified the approach of @Kooperberg1991 to allow for conditioning by minimising the $L_2$ distance from the score function to its conditional logspline approximation relative to the (empirical) bivariate density. We adapted the approach of @MasseTruong1999 in finding the initial parameters when the conditional density has a discrete support on the conditioning variable. When original data are used for the density estimation, and if the initial parameters produced by this method do not satisfy the tail constraints in \eqref{constrainedparametersconditional}, the constraints are imposed directly. When log data are used, and if the initial parameters do not satisfy the tail constraints in \eqref{newconstrainedparametersconditional}, the constraints are imposed directly.
...
\begin{equation}\label{method1}
  \hat{h} = \mathop{\text{arg}  \max}_{h}  l(h)
  \quad \text{where} \quad
  l(h) = \sum\limits_{t=1}^T L_t(h).
\end{equation}
...
$$
  r_{-t}(\bm{\theta},h)
    = \log \left\{ \int_\mathds{R} \exp \left(
      {\sum_{{\scriptstyle s=1 \atop \scriptstyle s\ne t}}^{T} \sum_{k=1}^{p} w_s(t) \theta_{s,k} B_k(y)} \right)  dy \right\}.
$$
Using leave-one-out log-likelihood functions over all $T$ data sets, $\{ L_t(h) \}_{t=1}^T$, the optimal bandwidth $\hat{h}$ can be found by maximising the sum of the leave-one-out log-likelihood functions as in \eqref{method1}.
...
\end{equation}
where the constants $A, B  \text{and}  D$ are given by:
$A = \sum\limits_{t=1}^T R^2(K) \label{AA}$,
$B = \sum\limits_{t=1}^T \left( \int_\mathds{R} R(K) f^2(y|t)  dy \right) \label{BB}$, and
$D = \sum\limits_{t=1}^T \left( \int_\mathds{R} \frac{\sigma_K^4 m_t}{4} \left\{ \frac{\partial^2 f(y|t)}{\partial y^2} \right\}^2  dy \right) \label{CC}$. Differentiating \eqref{IMSE2} with respect to $h_y$ and setting the derivatives to zero leads to the approximate optimal bandwidth of $h_y$:
...
\int_\mathds{R} \left( \hat{f}^{(2)}(y|t) \right)^2 dy = \frac{ \frac{1}{h_y^5} \mathop{\sum\limits_{s_2=1}^{T} \sum\limits_{s_1=1}^{T} \sum\limits_{j=1}^{n_{s_2}} \sum\limits_{i=1}^{n_{s_1}}}_{i \neq j} K \left( \frac{t-T'_{s_1}}{h_t} \right) K \left( \frac{t-T'_{s_2}}{h_t} \right) K^{(4)} \left( \frac{Y_{s_1,i} - Y_{s_2,j}}{h_y} \right) }
{ \left( \sum\limits_{s=1}^{T} n_s K \left( \frac{t-T'_s}{h_t} \right) \right)^2 }
\end{equation}
...
\int_\mathds{R} \left( \hat{f}^{(2)}(y|t) \right)^2 dy
  = \frac{
      \frac{1}{\sqrt{2\pi}{h_y}^5}
      \left[
        \sum\limits_{s_2=1}^{T}
        \sum\limits_{s_1=1}^{T}
        \sum\limits_{j=1}^{n_{s_2}}
        \sum\limits_{i=1}^{n_{s_1}}
          K \left( \frac{t-T'_{s_1}}{h_t} \right)
          K \left( \frac{t-T'_{s_2}}{h_t} \right) \zeta -
          3 \sum\limits_{s=1}^{T} n_s \left( K \left( \frac{t-T'_s}{h_t} \right) \right)^2
      \right]
    }{
      \left( \sum\limits_{s=1}^{T} n_s K \left( \frac{t-T'_s}{h_t} \right) \right)^2
    }
\end{equation}
where $\zeta = \exp(-\frac{z}{2})(z^2 - 6z + 3)$,
$z = \left( \frac{Y_{s_1,i} - Y_{s_2,j}}{h_y} \right)^2$,
$h_y = \sqrt{2}h'_y$,
$h'_y = \hat{\sigma}_Y \left( \frac{84\sqrt{\pi}}{5\bar{n}^2} \right)^{1/13}$,
$\hat{\sigma}_Y$ is the standard deviation of pooled $Y$ data,
$\bar{n}$ is the average number of observations and
$h_t$ is obtained from \eqref{h_t^*}.
With the integrated square of the second derivative of the density estimator of $f(y|t)$ in \eqref{plugin}, the optimal bandwidth in the $Y$ dimension, $\hat{h}_y$, is obtained from \eqref{h_y^*}.
...
\left\{ \{ u_{t,j} \}_{j=1}^{n_t} = \left\{ \int_{-\infty}^{y_{t,j}} \hat{f}_{t,j}(v) dv \right\}_{j=1}^{n_t} \right\}_{t=1}^T
\end{equation}
...
$\text{E} \left[ \frac{1}{h_t} K \left( \frac{t-T'}{h_t} \right) q(y|T') \right] = \sum\limits_{u} K(u) q(y|(t-uh_t)) m_t$ where $u=\frac{t-s}{h_t}$.
