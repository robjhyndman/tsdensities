...
The principal motivation of this paper arises due to the non-existence of previous studies on nonparametric estimation of a time series of densities, taking account of the time ordering of the densities. A few researchers have estimated a sequence of density functions, including \citet{KneipUtikal2001,ParkMarron1990,WandMarronRuppert1991}. However, they ignored the time ordering of the densities and estimated the densities independently for every particular year, ignoring all other years. This paper will address this issue by proposing a method to nonparametrically estimate a time series of density functions using logspline approach considering the time ordering of the densities.

We consider the logspline density estimation approach to estimate a time series of univariate density functions $\{f_t(y)\}_{t=1}^{T}$ with a similar structure using data $\{y_{t,j}\}$ for $t =1,\dots,T$ and $j=1,\dots,n_t$, where $n_t$ is the number of observations at time $t$ and $\{y_{t,j}\} \in \mathds{R}$. The logspline model was proposed by \citet{KooperbergStone1991, KooperbergStone1992, StoneHansenKooperberg1997}, and this model at time $t$ is given by:
\begin{equation}\label{usualmodel}
f_t(y;\bm{\theta}_t) = \text{exp} \left( b_t(y;\bm{\theta}_t) - c_t(\bm{\theta}_t) \right)
\end{equation}
where $\bm{\theta}_t = \{\theta_{t,k}\}_{k=1}^{p_t} \in \Theta$ is a vector of parameters, $b_t(y;\bm{\theta}_t) = \sum\limits_{k=1}^{p_t} \theta_{t,k} B_{t,k}(y)$, $c_t(\bm{\theta}_t) = \text{log} \left\{ \int_\mathds{R} \text{exp} \left( b_t(y;\bm{\theta}_t) \right) \hspace{0.1cm} \text{d}y \right\}$, $B_{t,k}(y)$ is the $k$th basis function at time $t$, $p_t = K_t - 1$ and $K_t$ is the number of knots at time $t$.

In $\eqref{usualmodel}$, we could treat every data set separately and a sequence of densities as a group of entirely independent curves that can be estimated separately. However, this is inefficient and ignores the time ordering of the densities. Instead, we consider estimating a time series of density functions using the logspline density approach with the same number and locations of knots at each time $t = 1,2,\dots,T$, where each estimated density has common knots but different parameters. Figure \ref{Est_Commonknotsv.Differentknots3} is drawn with the Australian income data between 1994/1995--2007/2008. The right plot is drawn with common knots and they are denoted using red tick marks. Densities in the right plot exhibit the consistent structure over time. This method with common knots across different density functions produces meaningful parameters and provides more information about the multiple densities than a situation with different knots.
...
\begin{figure}
  ...
  \begin{subfigure}{.5\textwidth}
    ...
    \vspace{1cm}
  \end{subfigure}
  \caption{Left plot with different knots versus right plot with common knots for the real example 3: Australian income 1994/1995--2007/2008 data set. Red tick marks in the right plot denote the locations of the common knots. The oldest years are shown in red and the most recent years are in blue. The curves are ordered chronologically according to the colours of the rainbow.}
  \label{Est_Commonknotsv.Differentknots3}
\end{figure}

All existing density estimation methods handle continuous data; however, few methods handle discrete data. \citet{LiRacine2003} considered the nonparametric estimation of joint distributions with mixed discrete and continuous data. \citet{HallRacineLi2004} considered the nonparametric estimation of conditional densities with mixed data and used cross-validation for choosing optimal bandwidths. This paper will address this issue---limited research on handling discrete data in density estimation methods---by proposing four conditional density estimation methods with time as a discrete conditioning variable.

The density evaluation methods in time series analysis are generally dealt with one observation at a time; however, this paper uses thousands of observations in each time period. To the best of our knowledge, the area of density evaluation with thousands of observations in each time period has not been touched upon in the literature. In light of the lack of attention paid to this topic, we implement a new method to evaluate the reliability of density estimates assessing calibration based on the probability integral transform (PIT).

In Section~\ref{ES1.1:sec:logsplinewithcommonknots}, we propose a spline-based method to estimate a time series of densities---A logspline approach is applied to each data set separately, where each estimated density has common knots but different parameters. A new conditional logspline approach is proposed in Section~\ref{ES1.2:sec:conditionallogsplinewithfullsplinesframework} allowing flexible adjustment of the smoothness of conditional density functions. The second conditional method is proposed in Section~\ref{EK1.3:sec:conditionallogsplinewithkernelweigths}, which combines two density estimation methods---logspline and kernel---and obtain conditional logspline density functions integrated with kernel weights. In Section~\ref{EK1.4:sec:conditionalkerneestimation}, we propose the conditional kernel estimator with two new bandwidth selection approaches. Section~\ref{APP:sec:application} applies the ideas on density estimation to six data sets. Then in section~\ref{APP:sec:evaluation}, we propose a new method to evaluate the reliability of density estimates and compares conditional density estimation methods using proper scoring rules. The computational time for each density estimation method is compared in Section~\ref{APP:sec:computationalefficiency}. Finally, the paper concludes by summarising the main findings and results.
...
f_t(y;\bm{\theta}_t) = \text{exp} \left( b_t(y;\bm{\theta}_t) - c_t(\bm{\theta}_t) \right)
\end{equation}
where $b_t(y;\bm{\theta}_t) = \sum\limits_{k=1}^{p} \theta_{t,k} B_k(y)$ and $c_t(\bm{\theta}_t) = \text{log} \left\{ \int_\mathds{R} \text{exp} \left( b_t(y;\bm{\theta}_t) \right) \hspace{0.1cm} \text{d}y \right\}$.
...
\subsection{Constrained optimisation and initial parameters}
\label{ES1.1:subsec:constrainedoptimization}
...
It is well known that optimisation algorithms converge to the solution quickly if the initial parameters are close to the optimal values. In finding the initial parameters, we follow \citet{Jin1990} and \citet{Kooperberg1991}. The idea behind constructing initial parameters is minimising the $L_2$ distance from the score function to its logspline approximation relative to the (empirical) density. We found the initial parameters solving the linear equation:
$U\hat{\bm{\eta}}^* = V$
...
In Section~\ref{ES1.1:sec:logsplinewithcommonknots}, we proposed a method using logspline density estimation to estimate the densities independently for every year, ignoring all other years. Under normal circumstances, it is reasonable to assume that these univariate densities are related to the ones in nearby years, and it is natural to account the nearby years to the density estimation of a particular year. This is the notion behind the `conditional density estimation'. \citet{MasseTruong1999} proposed a spline-based conditional logspline density estimation method which applies to bivariate data where both response and conditioning variables are continuous. In this section, we propose a modified version of the conditional method with two features: conditional density estimation with the discrete support of the conditioning variable, time $T'$; optimising spline degree---we not only choose the optimal knots but also optimise the spline degree of the density estimate. The concept of selecting the optimal spline degree for $B$-spline basis functions has also been used in the R package crs \citep{RacineNieRipley2014}.
...
\begin{equation}
\text{log} f(y|t;\bm{\alpha}) = \sum\limits_{k=0}^{p} \theta_k(t) B_k(y),
\end{equation}
$\theta_0(t) = -\text{log} \left\{ \int_\mathds{R} \text{exp} \left( \sum\limits_{k=1}^{p} \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y) \right) \hspace{0.1cm} \text{d}y \right\}
\hspace{0.3cm} \text{and} \hspace{0.3cm}
\theta_k(t) = \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t), \hspace{0.2cm} k=1,\dots,p$
where $\bm{\alpha} = \{\alpha_{j,k}\} \in \mathcal{A}$ are parameters for $j=0,\dots,q$ and $k=1,\dots,p$, and $\theta_0(t)$ is the normalising constant so that $\int_\mathds{R} f(y|t;\bm{\alpha}) \hspace{0.1cm} \text{d}y = 1$.

The log-likelihood function of the conditional logspline model with a discrete conditioning variable is defined by:
$
L(\bm{\alpha})  = \sum\limits_{t=1}^{T} \sum\limits_{i=1}^{n_t} \text{log} f(y_{t,i}|t;\bm{\alpha}) = \sum\limits_{t=1}^{T} \sum\limits_{i=1}^{n_t} \left( \sum\limits_{k=1}^{p} \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y_{t,i}) - a(\bm{\alpha}) \right)
$
where $a(\bm{\alpha}) = \text{log} \left\{ \int_\mathds{R} \text{exp} \left( \sum\limits_{k=1}^{p} \sum\limits_{j=0}^{q} \alpha_{j,k} G_j(t) B_k(y) \right) \hspace{0.1cm} \text{d}y \right\}$.

\subsection{Knots and spline degree selection}
\label{ES1.2:subsec:knotsselection}
...
\subsection{Constrained optimisation and initial parameters}
\label{ES1.2:subsec:constrainedoptimization}
...
\citet{MasseTruong1999} modified \citet{Kooperberg1991}'s approach into the conditional situation by minimising the $L_2$ distance from the score function to its conditional logspline approximation relative to the (empirical) bivariate density. We adapted \citet{MasseTruong1999}'s approach in finding the initial parameters when the conditional density has a discrete support on the conditioning variable. When original data are used for the density estimation, and if the initial parameters produced by this method do not satisfy the tail constraints in \eqref{constrainedparametersconditional}, the constraints are imposed directly. When log data are used, and if the initial parameters do not satisfy the tail constraints in \eqref{newconstrainedparametersconditional}, the constraints are imposed directly.
...
\label{EK1.3:subsec:optimisationmethods}
...
L_t(h) = \sum\limits_{i=1}^{n_t} \text{log} f(y_{t,i}|t;\bm{\theta},h)
\end{equation}
where $\text{log} f(y|t;\bm{\theta},h)$ is defined as in \eqref{objective1.3model}. This suggests that the optimal bandwidth $\hat{h}$ could be found by maximising the sum of the log-likelihood functions over all $T$ data sets:
...
We found that the maximum likelihood estimation has a trivial maximum at $h=0$. This implies that there is no difference between the conditional density estimates and the univariate density estimates. Therefore, the leave-one-out maximum likelihood estimation is invoked by replacing $\text{log} f(y_{t,i}|t;\bm{\theta},h)$ in \eqref{newL_t} by the leave-one-out logspline density estimation model at time $t$:
\begin{equation}
\log f_{-t}(y|t;\bm{\theta},h) = \sum\limits_{s(\ne t)=1}^{T} \sum\limits_{k=1}^{p} w_s(t) \theta_{s,k} B_k(y) - r_{-t}(\bm{\theta},h)
\end{equation}
where $r_{-t}(\bm{\theta},h) = \log \left\{ \int_\mathds{R} \exp \left( {\sum\limits_{s(\ne t)=1}^{T} \sum\limits_{k=1}^{p} w_s(t) \theta_{s,k} B_k(y)} \right) \hspace{0.1cm} \text{d}y \right\}$. Using leave-one-out log-likelihood functions over all $T$ data sets, $\{ L_t(h) \}_{t=1}^T$, the optimal bandwidth $\hat{h}$ could be found by maximising the sum of the leave-one-out log-likelihood functions as in \eqref{method1}.

# Conditional kernel estimation
\label{EK1.4:sec:conditionalkerneestimation}

Bandwidth selection is the most important aspect of the kernel density estimation. After the introduction of conditional kernel density, \citet{BashtannykHyndman2001} proposed several bandwidth selection approaches ranging from fast rule-of-thumb approaches to slow hi-tech approaches with continuous response and conditioning variables. In this section, we propose two bandwidth selection approaches for the conditional kernel estimation with the discrete conditioning variable.
...
$B = \sum\limits_{t=1}^T \left( \int_\mathds{R} R(K) f^2(y|t) \hspace{0.1cm} \text{d}y \right) \label{BB}$, and
$D = \sum\limits_{t=1}^T \left( \int_\mathds{R} \frac{\sigma_K^4 m_t}{4} \left\{ \frac{\partial^2 f(y|t)}{\partial y^2} \right\}^2 \hspace{0.1cm} \text{d}y \right) \label{CC}$. Differentiating \eqref{IMSE2} with respect to $h_y$ and setting the derivatives to zero leads to the approximate optimal bandwidth of $h_y$:
...
We have observed that $\hat{h}_t$ could not be found by differentiating \eqref{IMSE2} with respect to $h_t$. Alternatively, we use $\hat{h}_t$ expression of \citewiths{BashtannykHyndman2001} URR method. They assumed continuous conditioning variable is uniformly distributed over the sample space. Our conditioning variable is non-random discrete and uniformly distributed if approximately the same number of observations exist in each year. Therefore, assuming that the conditioning variable is uniformly distributed, the optimal bandwidth in the $T'$ dimension is found from \citet[][p. 286]{BashtannykHyndman2001}:
...
Following \citet{HallMarron1987} and \citet{WandMarronRuppert1991}, a new plug-in bandwidth selection approach is developed in this section. This approach plugs a value for the integrated squared derivatives of the conditional density where the unknown conditional density estimator is considered normal.
...
\int_\mathds{R} \left( \hat{f}^{(r)}(y|t) \right)^2 \text{d}y = \frac{ \frac{1}{\left( h_y^{2r+1} \right)^2} \mathop{\sum\limits_{s_2=1}^{T} \sum\limits_{s_1=1}^{T} \sum\limits_{j=1}^{n_{s_2}} \sum\limits_{i=1}^{n_{s_1}}}_{i \neq j} K \left( \frac{t-T'_{s_1}}{h_t} \right) K \left( \frac{t-T'_{s_2}}{h_t} \right) \int_\mathds{R} K^{(r)} \left( \frac{y - Y_{s_1,i}}{h_y} \right) K^{(r)} \left( \frac{y - Y_{s_2,j}}{h_y} \right) \text{d}y }
{ \left( \sum\limits_{s=1}^{T} n_s K \left( \frac{t-T'_s}{h_t} \right) \right)^2 }
\end{equation}
where $K^{(r)}(\cdot)$ is the $r$ derivative of the kernel function.

Following \citewiths{HallMarron1987} idea in univariate density estimation, we propose the integrated squared second derivative of the conditional kernel density estimator of $f(y|t)$:
\begin{equation}\label{wand}
\int_\mathds{R} \left( \hat{f}^{(2)}(y|t) \right)^2 \hspace{0.2cm} \text{d}y = \frac{ \frac{1}{h_y^5} \mathop{\sum\limits_{s_2=1}^{T} \sum\limits_{s_1=1}^{T} \sum\limits_{j=1}^{n_{s_2}} \sum\limits_{i=1}^{n_{s_1}}}_{i \neq j} K \left( \frac{t-T'_{s_1}}{h_t} \right) K \left( \frac{t-T'_{s_2}}{h_t} \right) K^{(4)} \left( \frac{Y_{s_1,i} - Y_{s_2,j}}{h_y} \right) }
{ \left( \sum\limits_{s=1}^{T} n_s K \left( \frac{t-T'_s}{h_t} \right) \right)^2 }
\end{equation}
...
\int_\mathds{R} \left( \hat{f}^{(2)}(y|t) \right)^2 \hspace{0.1cm} \text{d}y =
...
\subsection{Logspline with common knots}
...
\subsection{Conditional logspline with adaptive polynomial framework}
\label{APP:subsec:conditionallogsplinewithadaptivepolynomialframework}
...
\subsection{Conditional logspline with kernel weights}
...
\subsection{Conditional kernel estimation}
...
Let $f_t(y)$ and $\hat{f}_t(y)$ be the true density and estimated density at time $t$ respectively. Each density has thousands of observations. Given that the estimates should be evaluated by assessing whether the estimated densities are correct, we test whether the collection of estimated densities is equal to the collection of true densities:
$
\{ f_t(y) \}_{t=1}^T = \{ \hat{f}_t(y) \}_{t=1}^T.
$
...
\left\{ \{ u_{t,j} \}_{j=1}^{n_t} = \left\{ \int_{-\infty}^{y_{t,j}} \hat{f}_{t,j}(v) \hspace{0.2cm} \text{d}v \right\}_{j=1}^{n_t} \right\}_{t=1}^T
\end{equation}
...
The significance of the scores is statistically tested using the Diebold--Mariano test \citep[][p. 137]{GneitingKatzfus2014}. This is a paired t-test that can be used to test for equal performance of two competing reliable density estimation methods. By performing paired t-tests, we can conclude that two conditional logspline methods do not differ in performance in producing sharp density estimates; however, they perform better than four bandwidth selection methods in conditional kernel estimation.
...
\subsection{Lemma}\label{ourlemma}
...
$
where $q_1(T'_s,Y_{s,j})$ and $q_2(T'_s)$ are two random variables with means $\mu_1$ and $\mu_2$ and variances $\sigma_1^2$ and $\sigma_2^2$ respectively, and with the covariance $\sigma_{12}^2$. Following Lemma 2 in \citet[][pp. 333-335]{HyndmanBashtannykGrunwald1996}, the expected value and the variance of the estimator $\hat{f}(y|t)$ can be taken as:
...
Then applying Lemma 1 in \citet[][pp. 331-332]{HyndmanBashtannykGrunwald1996} gives:
...
To find $V_2$, firstly, apply Lemma 1 in \citet[][pp. 331-332]{HyndmanBashtannykGrunwald1996} for the variance conditioning on $T'_s$. Then the expectation of the result gives $V_2$: \\
$
...
Conditioning on $T'_s$ and applying Lemma 1 in \citet[][pp. 331-332]{HyndmanBashtannykGrunwald1996} gives:
$
